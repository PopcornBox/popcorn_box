		...
		<div class="container">
			<div class="contact__form">
				...
				<form>
					...
					<div class="contact__form" id="board_title" name="board_title">${board.board_title}
					</div>
					...
					<div class="row">
						...
						<c:if test="${signInUserNickname == board.user_nickname}">
							<div style="margin-left: auto; margin-right: 30px;">
								...
								<ul class="my_board_menu"
									style="display: inline-flex; list-style: none; font-size: 14px;">

									<li><a href="./update?board_no=${board.board_no}">글 수정</a>
									</li>
									<li><a id="menu-delete"
						...
						<c:if test="${signInUserPosition eq 'A'}">
							<div style="margin-left: auto; margin-right: 30px;">
								...
								<ul
									...
									<li><a id="menu-ntdelete"
										href="./delete?board_no=${board.board_no}">관리자 권한 글 삭제</a></li>
								</ul>
							</div>
						</c:if>
						<c:if test="${signInUserPosition eq 'B'}">
							<div style="margin-left: auto; margin-right: 30px;">
								...
								<ul
									style="display: inline-flex; list-style: none; font-size: 14px;">
									<li><a id="menu-ntdelete"
										href="./delete?board_no=${board.board_no}">관리자 권한삭제</a></li>
								</ul>
							</div>
						</c:if>
						<c:if test="${signInUserPosition eq 'B'}">
							<div style="margin-left: auto; margin-right: 30px;">
								...
								<ul
									style="display: inline-flex; list-style: none; font-size: 14px;">
									<li><a id="menu-update"
										href="./update?board_no=${board.board_no}">관리자 권한수정</a></li>
								</ul>
							</div>
						</c:if>
					...
					<hr>
					<div>
						<div style="white-space: pre-wrap" id="board_content"
							name="board_content" required readonly>${board.board_content}</div>
					</div>
				</form>
				<hr>
				...
				<div style="display: flex;">
					...
					<button id="btn_create_boardReply" class="primary-btn"
						style="color: #fff">
						댓글<br>입력
					</button>
				...
				.ready(
						function() {
							...
							$('#board_reply_content')
									.click(
											function() {
												if ('${signInUserNickname}' == null
														|| '${signInUserNickname}' == '') {
													var message = '로그인이 필요한 서비스입니다. 로그인 페이지로 이동하시겠습니까?';
													var result = confirm(message);
													if (result == true) {
														location.href = '/pjt/user/signin?board_no='
																+ $
														{
															board.board_no
														}
